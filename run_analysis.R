...
# now excerpt mean, std  (#2)
#rnames= c(1,2,grep("mean",tnames),grep("Mean",tnames),grep("std",tnames))
redtot = total[,c(1,2,grep("mean",tnames),grep("Mean",tnames), grep("std",tnames))]
...
#now aggregate  (#5)
actions =aggregate(redtot[3:88],by=list(redtot$subject, redtot$activity),FUN=mean)
...
# por fin
write.table(actions, file = "actions2.txt", row.name = FALSE)
